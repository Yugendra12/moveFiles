...
async function uploadToGit() {
  files.forEach((file) => {
    ...
    if (file.endsWith(".feature")) {
      ...
      if (!fs.existsSync(destFeatureFilePath)) {
        ...
        console.log(
          `Feature file automatically moved to destination folder: ${file}`
        );
      } else {
    ...
    if (file.endsWith(".java")) {
      ...
      if (!fs.existsSync(destJavaFilePath)) {
        ...
        console.log(
          `Java file automatically moved to destination folder: ${file}`
        );
      } else {
        console.log(`File already exists in destination: ${file}`);
      }
  ...
  async function checkGitRepo() {
    try {
      ...
      if (!isRepo) {
        console.log(
          "Not a Git repository. Initializing a new Git repository..."
        );
  ...
  async function gitProcess() {
    ...
    await git.commit(
      "Added .feature and .java files to features and step-definitions"
    );
